...
task("spawn-collection", "Creates a new NFT collection", async (taskArgs, hre) => {
    const factoryAddress = (await hre.deployments.get("NFTCollectionFactory")).address;
    console.log(`NFTCollectionFactory address: ${factoryAddress}`);
    const nftCollectionFactory = await hre.ethers.getContractAt("NFTCollectionFactory", factoryAddress);
    const tx = await nftCollectionFactory.createCollection("MyCollection", "MCL");
